function bounds = setBounds(rabbit)

%% Set bounds for optimization problem

% Set Bounds
model_bounds = rabbit.getLimits();
model_bounds.states.x.lb = [-10, 0.5, 0, 2, 0, 2, 0];
model_bounds.states.x.ub = [10, 2, 1, 5, 1, 5, 1];
...
bounds.RightStance.time.tf.lb = 0;
bounds.RightStance.time.tf.ub = 1;
bounds.RightStance.time.tf.x0 = 0.5;

bounds.RightStance.time.duration.lb = 0;
