...
%     dx = domain.States.dx;
    ...
    % add parameter for reference trajectory
    wref = SymVariable('wref',[1,1]);
    domain = addParam(domain,'wref',wref);
    
    xref = SymVariable('xref',[7*21,1]);
    domain = addParam(domain,'xref',xref);
