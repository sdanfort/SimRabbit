function [out] = invariantSetHybridBang2Q(x,f,gu,gd,sX,R,hX,hXF,lambda_0,sigma_0,c_0,u_0,dl,degree,x_TP,TP_weight,x_Con,plotFun,sExp)
%
...
%             lambda = lambdaNew;
            ...
            [lambdaNew, cNew, uNew] = lOPT(prob, v, rho, i == 1);
...
function [lambdaOut, cOut, uOut] = lOPT(prob, v, rho, init)
    %% lambda optimization
    %% Unpack problem
    x = prob.x;
    f = prob.f;
    ...
    persistent progSaved e lambda q c u obj uSel
...
%         c = 0;
        ...
        for i = 1:nModes
            ...
            if isempty(gd{i})
                ...
                q{i} = msspoly( zeros( [size( gd{i}, 2 ), 1] ) );
                for j = 1:size(gd{i},2)
                    [ progSaved, q{i}(j) ] = progSaved.newFreePoly( lmonom );
                end
            end
        end
