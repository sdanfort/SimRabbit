function [out] = invariantSetDerivDelta(x,f,gu,gd,hX,hXF,lambda_0,c_0,u_0,dl,degree,x_TP,TP_weight,dudtMax,deltaV)
%
...
%         prob.TP_weight = 1000;
    ...
    prob.degree = degree;
    if nargin < 14
        dudtMax = inf(size(gu,2));
    end
    prob.dudtMax = dudtMax;
    if nargin < 15
        deltaV = 1;
    end
    prob.deltaV = deltaV;
...
%         [imonom,icoeff] = indFun(lambda,x,hX,dl,degree,options);
        ...
        [vNew, qNew, objNew, area] = vOPT(prob, lambda, v, imonom, icoeff, c, u, i == 1);
        ...
        if (c <= 0) && vMax > eps && (objNew < (1+1e-3)*obj) && i >= minIter
...
function [vOut,qOut,objOut,areaOut] = vOPT(prob, lambda, vOld, imonom, icoeff, c, u, init)
    %% v optimization
    %% Unpack problem
    x = prob.x;
    f = prob.f;
    ...
    % v < 0 when vOld < -deltaV 
    prog = sosOnK(prog,-v_, x, [hX; -vOld-deltaV], degree);
    
    % dv_dt > 0 on the barrier {x | v(x) = 0}
...
function [lambdaOut, cOut, uOut] = lOPT(prob, v, q, init)
    ...
    persistent progSaved_ e_ lambda_ c_ u_ obj_ qUu_ qUd_
    ...
    if init
        ...
        if isempty(gu)
            ...
            u_ = msspoly( zeros(nu, 1) );
            qUu_ = msspoly( zeros(nu, nu) );
            qUd_ = msspoly( zeros(nu, nd) );
            for i = 1:nu
                [ progSaved_, u_(i) ] = progSaved_.newFreePoly( pmonom_ );
                
                if ~isinf(dudtMax(i))
                    % There is a derivative constraint on u_(i)
                    for j = 1:nu
                        [ progSaved_, qUu_(i,j) ] = progSaved_.newFreePoly( pmonom_ );
                    end
                    for j = 1:nd
                        [ progSaved_, qUd_(i,j) ] = progSaved_.newFreePoly( pmonom_ );
                    end
                end
            end
        end
    ...
    % Control inputs are inside [-1,1] in the safe set
    for i = 1:nu
        prog = sosOnK(prog, 1 - u_(i), x, [hX;v+deltaV], degree);
        prog = sosOnK(prog, 1 + u_(i), x, [hX;v+deltaV], degree);
    end

    % Control input derivatives are inside [-epsU, epsU] in the safe set
    for i = 1:nu
        if ~isinf(dudtMax(i))
            dudx_ = diff( u_(i), x );
            Lfu_ = dudx_ * f;
            ...
            for j = 1:nu
                prog = sosOnK(prog, qUu_(i,j) - Lguu_(j), x, [hX;v+deltaV], degree);
                prog = sosOnK(prog, qUu_(i,j) + Lguu_(j), x, [hX;v+deltaV], degree);
            end
            for j = 1:nd
                prog = sosOnK(prog, qUd_(i,j) - Lgdu_(j), x, [hX;v+deltaV], degree);
                prog = sosOnK(prog, qUd_(i,j) + Lgdu_(j), x, [hX;v+deltaV], degree);
            end

            prog = sosOnK(prog, dudtMax(i) - Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)), x, [hX;v+deltaV], degree);
            prog = sosOnK(prog, dudtMax(i) + Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)), x, [hX;v+deltaV], degree);
        end
    end
