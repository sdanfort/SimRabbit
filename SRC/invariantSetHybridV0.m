function [out] = invariantSetHybridV0(x,f,gu,gd,sX,R,hX,hXF,v_0,sigma_0,c_0,dl,degree,x_TP,TP_weight,x_Con,plotFun)
%
...
%          indefinitely.
    ...
    %% Run a first iteration of the lambda optimization
    [lambdaNew, c, uNew] = lOPT(prob, v);
...
%         icoeff = repcell({1},nModes,1);
        ...
        [vNew, qNew, objNew, area, c] = vOPT(prob, lambdaNew, sigma, c, uNew, i == 1);
...
function [vOut,qOut,objOut,areaOut,c] = vOPT(prob, lambda, sigma, c, u, init)
    %% v optimization
    ...
    n = 1;
    maxIter = 10;
    while sol.primalInfeasible && n <= maxIter
        % Problem infeasible with the current c, increase c and try again
        if c <= 0
            c = 0.05;
        else
            c = 2*c;
        ...
        fprintf('Primal infeasible, increasing c to %4.2f\n',c)
...
%         c = 0;
    ...
    if nargin < 3
        % Add the qs to the optimization
        q = cell(nModes,1);
        for i = 1:nModes
            dvdx = diff( v{i}, x{i} );
            ...
            if isempty(gd{i})
                ...
                q{i} = msspoly( zeros( [size( gd{i}, 2 ), 1] ) );
                for j = 1:size(gd{i},2)
                    [ prog, q{i}(j) ] = prog.newFreePoly( qmonom );
                end
            ...
            % Disturbances are outside [-1,1]
            for j = 1:size( gd{i}, 2 )
                prog = sosOnK(prog, q{i}(j) - Lgdv(j), x{i}, hX{i}, degree);
                prog = sosOnK(prog, q{i}(j) + Lgdv(j), x{i}, hX{i}, degree);
            end
        end
    end
