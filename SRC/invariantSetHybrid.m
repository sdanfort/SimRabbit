function [out] = invariantSetHybrid(x,f,gu,gd,sX,R,hX,hXF,lambda_0,sigma_0,c_0,u_0,dl,degree,x_TP,TP_weight,x_Con,plotFun,sExp,convIter,inputScale)
%
...
%          indefinitely.
    ...
    if ~exist('convIter','var')
        convIter = 1;
    end
    if ~exist('inputScale','var')
        prob.inputScale = 1;
    else
        prob.inputScale = inputScale;
    end
...
%         icoeff = repcell({1},nModes,1);
        ...
        %Here, lambda and u are known, finding v and q.
        [vNew, qNew, objNew, areaNew] = vOPT(prob, lambdaNew, sigma, c, uNew, i == 1);

        %find maximum v-value across all modes and return
        vMaxNew = polyMax(vNew,x,hX,degree,options);
        
        fprintf('V found for c = %4.4f with objective %4.2f, area %4.2f and max value %4.4f\n',...
            c, objNew, areaNew, vMaxNew)
        ...
        if ( (i > convIter) && (convFlag || (c <= 0)) && ( (objNew < obj + epsConv*abs(obj)) ) ) % || (vMaxNew <= epsConv) ) )
...
%             lambda = lambdaNew;
            ...
            %Now v and q are known, finding lambda and u
            [lambdaNew, cNew, uNew] = lOPT(prob, v, q, i == 1);
...
function [vOut,qOut,objOut,areaOut] = vOPT(prob, lambda, sigma, c, u, init)
    ...
    for i = 1:nModes
        ...
        prog = sosOnK(prog,Lfv{i} - e{i}'*q{i} + Lguv{i}*u{i} +...
            lambda{i}*v{i} + c - epsBar, x{i}, hX{i}, degree);
...
%         c = 0;
        ...
        for i = 1:nModes
            ...
            for j = 1:size( gu{i}, 2 )
                progSaved = sosOnK(progSaved, 1*inputScale - u{i}(j), x{i}, hX{i}, degree);
                progSaved = sosOnK(progSaved, 1*inputScale + u{i}(j), x{i}, hX{i}, degree);
            end
        end
    ...
    for i = 1:nModes
        ...
        prog = sosOnK(prog, Lfv - e{i}'*q{i} + Lguv*u{i}  + lambda{i}*v{i} +...
