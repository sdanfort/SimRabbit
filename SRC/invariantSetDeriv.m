function [out] = invariantSetDeriv(x,f,gu,gd,hX,hXF,lambda_0,c_0,u_0,dl,degree,x_TP,TP_weight,dudtMax)
%
    ...
    if init
        ...
        for i = 1:nu
            if ~isinf(dudtMax(i))
                ...
                for j = 1:nu
                    progSaved_ = sosOnK(progSaved_, qUu_(i,j) - Lguu_(j), x, hX, degree);
                    progSaved_ = sosOnK(progSaved_, qUu_(i,j) + Lguu_(j), x, hX, degree);
                end
                for j = 1:nd
                    progSaved_ = sosOnK(progSaved_, qUd_(i,j) - Lgdu_(j), x, hX, degree);
                    progSaved_ = sosOnK(progSaved_, qUd_(i,j) + Lgdu_(j), x, hX, degree);
                end

                progSaved_ = sosOnK(progSaved_, dudtMax(i) - Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)) + c_, x, hX, degree);
                progSaved_ = sosOnK(progSaved_, dudtMax(i) + Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)) + c_, x, hX, degree);
            end
        end
