function [t,x,m,exitFlag] = simulateHybridNondtr(f, hS, R, hSBar, x0, m0, tRange, hX, hXF, dim,maxstep)
...
for i = 1:maxEvents
    ...
    IE_0_ = find(eventFun(x0_(:),hS(m_,:),hX{m_},hXF{m_})>=0);
    if ~isempty(IE_0_)
        if min(IE_0_) <= nModes 
            % Hit a guard
            mPLUS = min(IE_0_);
...
%             return
    ...
    if nargin < 10
        opts = odeset('events',@(~,x_) eventFun(x_(:),hS(m_,:),hX{m_},hXF{m_}));%,...
%                       'abstol',1e-3,'reltol',1e-2);
                  %'maxstep',1e-3);
    else
        opts = odeset('events',@(~,x_) eventFun(x_(:),hS(m_,:),hX{m_},hXF{m_}),'maxstep',maxstep);
    end
...
function [value, isterminal, direction] = eventFun(x,S,hX,hXF)
