...
%         [imonom,icoeff] = indFun(lambda,x,hX,dl,degree,options);
        ...
        [v, q, objNew, area] = vOPT(prob, lambda, imonom, icoeff, c, u, i == 1);
        vMax = polyMax(v,x,hX,degree,options);
        ...
        if (c <= 0) && vMax > eps && (objNew < (1+1e-5)*obj) && i > minIter
...
function [lambdaOut, cOut, uOut] = lOPT(prob, v, q, init)
    ...
    for i = 1:nu
        progSaved_ = sosOnK(progSaved_, 1 - u_(i), x, [hX;v], degree);
        progSaved_ = sosOnK(progSaved_, 1 + u_(i), x, [hX;v], degree);
    end
    ...
    for i = 1:nu
        if ~isinf(dudtMax(i))
            ...
            for j = 1:nu
                progSaved_ = sosOnK(progSaved_, qUu_(i,j) - Lguu_(j), x, [hX;v], degree);
                progSaved_ = sosOnK(progSaved_, qUu_(i,j) + Lguu_(j), x, [hX;v], degree);
            end
            for j = 1:nd
                progSaved_ = sosOnK(progSaved_, qUd_(i,j) - Lgdu_(j), x, [hX;v], degree);
                progSaved_ = sosOnK(progSaved_, qUd_(i,j) + Lgdu_(j), x, [hX;v], degree);
            end

            progSaved_ = sosOnK(progSaved_, dudtMax(i) - Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)), x, [hX;v], degree);
            progSaved_ = sosOnK(progSaved_, dudtMax(i) + Lfu_ - sum(qUu_(i,:)) - sum(qUd_(i,:)), x, [hX;v], degree);
        end
    end
